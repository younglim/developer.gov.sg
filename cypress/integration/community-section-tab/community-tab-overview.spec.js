describe("Community Section Overview Page Test", function() {
  context("Desktop Test Case", () => {
    ...
    it("Should only display less than or equal to 2 content for all upcoming events.", async () => {
  ...
  context("Mobile Test Case", () => {
    ...
    it("Should display the message, 'Sorry, there is currently no upcoming available events.' when there are no upcoming events", async () => {
      cy.get("#upcoming-events-container").then(($body) => {
        // if no events, then check if the no message text is displayed
        if ($body.find("div[data-future-date]").length == 0) {
          cy.get("div[id='no-upcoming-events-message']").should("be.visible");
    ...
    it("Should display the 'View all events' button even if there is no upcoming events.", async () => {
      cy.contains("Upcoming Events").scrollIntoView();
    ...
    it("Should navigate all events seach page after clicking on the 'View all events' button", async () => {
      cy.get("#upcoming-conferences-button")
        .contains("View all events")
        .click();
    ...
    it("Should display at least 3 newsletter.", async () => {
      // Reset
      cy.visit("http://localhost:4000/communities");
    ...
    it("Should be able to click on the “View All Newsletter” that will display more information. Should be greater than the initial number.", async () => {
      // Click on the view more content
      cy.get("#newsletter-display-more")
        .scrollIntoView()
        .click();
    ...
    it("Should hide the button after clicking on the “View All Newsletter” button", async () => {
      // Scroll into view
      cy.get("#tab1")
        .get(".card-grid-container")
        .get(".newsletter-card")
